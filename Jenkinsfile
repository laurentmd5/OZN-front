                    ...
                    steps {
                        script {
                            try {
                                ...
                                WARNING_COUNT=$(grep -c "warning •" "${SAST_DIR}/flutter_analysis.txt" 2>/dev/null || echo "0")
                                INFO_COUNT=$(grep -c "info •" "${SAST_DIR}/flutter_analysis.txt" 2>/dev/null || echo "0")
                                ...
                                # Échec seulement si des erreurs critiques
                                if [ ${ERROR_COUNT} -gt 0 ]; then
                                    echo "❌ Critical errors found"
                                    grep "error •" "${SAST_DIR}/flutter_analysis.txt" || true
                                    exit 1
                                fi
