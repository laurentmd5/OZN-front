pipeline {
    ...
    stages {
        ...
        stage('Flutter Analyze & SAST') {
            steps {
                sh '''
                echo "🔍 Running combined Flutter Analyze & SAST..."
                mkdir -p ${SAST_REPORTS_DIR}/metrics
                mkdir -p ${SAST_REPORTS_DIR}/security

                # Analyse Flutter (warnings non bloquants)
                flutter analyze --no-pub --current-package --write=${SAST_REPORTS_DIR}/flutter_analysis.json || echo "⚠️ Flutter analyze completed with warnings"

                # Export analyse détaillée en TXT
                ...
                echo "✅ Flutter Analyze & SAST completed (non-blocking)"
                '''
            }
        }
